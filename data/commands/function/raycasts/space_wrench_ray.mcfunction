...
execute if entity @s[team=Blue] if score #hit ray.wrench matches 0 unless score #hitentity ray.wrench matches 1 positioned ~-0.05 ~-0.05 ~-0.05 as @a[tag=!wrenchUser,sort=nearest,distance=..4,team=Blue] if score #hit ray.wrench matches 0 positioned ~-0.85 ~-0.85 ~-0.85 if entity @s[dx=0] at @s run function commands:raycasts/space_wrench_hit_entity
