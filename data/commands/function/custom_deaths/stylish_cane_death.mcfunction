...
execute if score #rng RNG matches 1 on attacker run tellraw @a [{"selector":"@a[advancements={commands:custom_deaths/stylish_cane_death=true}]"},{"text":" was jabbed too hard by ","color":"white"},{"selector":"@s"}]

execute if score #rng RNG matches 2 run tellraw @a [{"selector":"@s"},{"text":" lost a bet and was beaten to death.","color":"white"}]

execute if score #rng RNG matches 3 on attacker run tellraw @a [{"selector":"@a[advancements={commands:custom_deaths/stylish_cane_death=true}]"},{"text":" was poked too hard by ","color":"white"},{"selector":"@s"}]
