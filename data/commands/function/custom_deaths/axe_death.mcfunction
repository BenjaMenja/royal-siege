...
execute if score #rng RNG matches 4 on attacker run tellraw @a [{"selector":"@a[advancements={commands:custom_deaths/axe_death=true}]"},{"text":" was hacked to bits by ","color":"white"},{"selector":"@s"}]

execute if score #rng RNG matches 5 on attacker run tellraw @a [{"selector":"@a[advancements={commands:custom_deaths/axe_death=true}]"},{"text":" was bludgeoned by ","color":"white"},{"selector":"@s"}]

execute if score #rng RNG matches 6 on attacker run tellraw @a [{"selector":"@a[advancements={commands:custom_deaths/axe_death=true}]"},{"text":" was hit with some good ol' blunt force trauma by ","color":"white"},{"selector":"@s"}]
