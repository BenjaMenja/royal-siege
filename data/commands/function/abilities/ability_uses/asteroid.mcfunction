execute at @s[team=Red] anchored eyes run summon minecraft:fireball ^ ^ ^1 {HasVisualFire:0b,ExplosionPower:3b,Tags:["asteroid","redProj","fm"],Motion:[0.0,0.0,0.0],Passengers:[{id:"minecraft:marker",Tags:["asteroidLand"]}],Item:{id:"minecraft:carrot",count:1,components:{"minecraft:item_model":"royalsiege:asteroid"}}}

execute at @s[team=Blue] anchored eyes run summon minecraft:fireball ^ ^ ^1 {HasVisualFire:0b,ExplosionPower:3b,Tags:["asteroid","blueProj","fm"],Motion:[0.0,0.0,0.0],Passengers:[{id:"minecraft:marker",Tags:["asteroidLand"]}],Item:{id:"minecraft:carrot",count:1,components:{"minecraft:item_model":"royalsiege:asteroid"}}}
...
execute as @e[tag=asteroid,tag=!stop] store result entity @s Motion[0] double 0.0003 run scoreboard players get @s posX

execute as @e[tag=asteroid,tag=!stop] store result entity @s Motion[1] double 0.0003 run scoreboard players get @s posY

execute as @e[tag=asteroid,tag=!stop] store result entity @s Motion[2] double 0.0003 run scoreboard players get @s posZ
