execute at @s anchored eyes run summon minecraft:item ^ ^ ^1 {PickupDelay:32767,Tags:["fishCannon","fishCannonBlue","blueProj"],Item:{id:"minecraft:cod",count:1}}
...
scoreboard players operation @e[type=item,sort=nearest,limit=1,tag=fishCannonBlue,tag=!stop] posX -= @s posX

scoreboard players operation @e[type=item,sort=nearest,limit=1,tag=fishCannonBlue,tag=!stop] posY -= @s posY

scoreboard players operation @e[type=item,sort=nearest,limit=1,tag=fishCannonBlue,tag=!stop] posZ -= @s posZ

execute as @e[type=item,sort=nearest,limit=1,tag=fishCannonBlue,tag=!stop] store result entity @s Motion[0] double 0.0064 run scoreboard players get @s posX

execute as @e[type=item,sort=nearest,limit=1,tag=fishCannonBlue,tag=!stop] store result entity @s Motion[1] double 0.0064 run scoreboard players get @s posY

execute as @e[type=item,sort=nearest,limit=1,tag=fishCannonBlue,tag=!stop] store result entity @s Motion[2] double 0.0064 run scoreboard players get @s posZ
