...
execute at @s[team=Red] as @a[distance=..15,team=Red] run function commands:utils/heal/custom_heal {health:4,ult_charge_receiver:"@a[tag=blossoming]"}
...
execute at @s[team=Blue] as @a[distance=..15,team=Blue] run function commands:utils/heal/custom_heal {health:4,ult_charge_receiver:"@a[tag=blossoming]"}
