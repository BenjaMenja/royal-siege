...
execute as @e[type=item] if data entity @s Item.components.minecraft:custom_data.rocket_death store result score #rng RNG run data get entity @s Item.components.minecraft:attribute_modifiers.modifiers[{name:"RNG"}].amount
kill @e[type=item,nbt={Item:{components:{"minecraft:custom_data":{rocket_death:1b}}}}]
