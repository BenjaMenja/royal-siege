execute at @s[scores={Treeremove=1..2}] run effect give @a[team=Blue,distance=..10] regeneration 10 3 true

execute at @s[scores={Treeremove=1..2}] run effect give @a[team=Blue,distance=..10] absorption 10 1 true

execute at @s[scores={Treeremove=1},tag=TreeoflifeBlue] run playsound royalsiege:ultimates.tree_of_life_friendly master @a[team=Blue] ~ ~ ~ 100 1
