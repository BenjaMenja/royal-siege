...
execute if entity @s[tag=ovenred] if entity @a[tag=inLOS,team=Blue] at @s anchored eyes run summon minecraft:fireball ^ ^ ^ {ExplosionPower:4b,Tags:["pizzared","pizza","redProj"],Item:{id:"minecraft:pumpkin_pie",Count:1b,tag:{CustomModelData:136}}}

execute if entity @s[tag=ovenblue] if entity @a[tag=inLOS,team=Red] at @s anchored eyes run summon minecraft:fireball ^ ^ ^ {ExplosionPower:4b,Tags:["pizzablue","pizza","blueProj"],Item:{id:"minecraft:pumpkin_pie",Count:1b,tag:{CustomModelData:136}}}
...
data modify entity @e[type=item_frame,limit=1,sort=nearest] Item set value {id:"minecraft:stone_bricks",Count:1b,tag:{CustomModelData:135}}
