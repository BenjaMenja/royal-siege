...
execute if entity @s[tag=blazingspeedbombred] as @a[distance=..3,team=Blue] at @s run summon small_fireball ~ ~2.1 ~ {Fire:20s,Motion:[0.0d,-1.0d,0.0d]}

execute if entity @s[tag=blazingspeedbombblue] as @a[distance=..3,team=Red] run damage @s 5 commands:bang_snap

execute if entity @s[tag=blazingspeedbombblue] as @a[distance=..3,team=Red] at @s run summon small_fireball ~ ~2.1 ~ {Fire:20s,Motion:[0.0d,-1.0d,0.0d]}
